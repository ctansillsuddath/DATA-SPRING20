...
The r we've calculated is positive: as pills per person increases, so too does the death rate (and vice versa).
...
# Store a dataframe with two columns, death rate, and pills rate. 
correlation_pills_income <- pills_population_income %>%
  ungroup() %>%
  select(median_household_income, average_pills_per_person) 

# Test the relationship
cor.test(correlation_pills_income$median_household_income, correlation_pills_income$average_pills_per_person)


```
```{R}
# This data gives us a P-value of 2.2e-16 which, for our purposes, is significant. This means a null hypothesis—or the idea that the two variables are random—is likely incorrect. My data analysis is most likely accurate.

#P.S. Sean, this is the first labI was able to complete without help from the tutorial :)
